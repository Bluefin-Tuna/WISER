...
class Text2WorldModelRectifiedFlow(ImaginaireModel):
    ...
    def get_velocity_Mx_from_batch(
        self,
        data_batch: Dict,
        guidance: float = 1.5,
        is_negative_prompt: bool = False,
        ...
        def velocity_Mx_fn(noise: torch.Tensor, noise_x: torch.Tensor, timestep: torch.Tensor) -> torch.Tensor:
            # cond_v = self.denoise(noise, noise_x, timestep, condition)
            uncond_v = self.denoise(noise, noise_x, timestep, uncondition)
            velocity_pred = uncond_v #+ guidance * (cond_v - uncond_v)
            return velocity_pred

        return velocity_Mx_fn

    @torch.no_grad()
    def generate_samples_from_batch_safeguard(
        self,
        data_batch: Dict,
        guidance: float = 1.5,
        seed: int = 1,
        ...
        **kwargs,
        ...
        velocity_Mx_fn = self.get_velocity_Mx_from_batch(data_batch, guidance, is_negative_prompt=is_negative_prompt)
        ...
        if INTERNAL:
            ...
            timesteps_iter = tqdm.tqdm(timesteps, desc="Generating samples with M(x)", total=len(timesteps))

        #I need a way to check the conditioned latents before they are passed in....
        #There is a non conditioned setting.... 
        for _, t in enumerate(timesteps_iter):
            ...
            #The latents are the end of the block and denoiser fills up the block 1 by one.
    ...
    @torch.no_grad()
    def denoise_from_reference(
        self,
        data_batch: Dict,
        noise_strength: float = 0.5,  # NEW: how much noise to add
        guidance: float = 1.5,
        seed: int = 1,
        num_steps: int = 35,
        shift: float = 5.0,
        **kwargs,
        ...
        """
        Modified version that starts from reference image + noise instead of pure noise.
        """
        self._normalize_video_databatch_inplace(data_batch)
        self._augment_image_dim_inplace(data_batch)
        is_image_batch = self.is_image_batch(data_batch)
        ...
        reference_image = data_batch[input_key]
        latents_clean = self.tokenizer.encode(reference_image)
        ...
        # Mix clean latents with noise based on noise_strength
        # noise_strength=0.0 -> pure clean image
        # noise_strength=1.0 -> pure noise
        latents = (1 - noise_strength) * latents_clean + noise_strength * noise
        ...
        # Start from timestep corresponding to noise_strength
        start_idx = int(noise_strength * len(timesteps))
        timesteps = timesteps[start_idx:]
        
        velocity_fn = self.get_velocity_fn_from_batch(
            data_batch, guidance, is_negative_prompt=False
        )
        
        if self.net.is_context_parallel_enabled:
            latents = broadcast_split_tensor(
                tensor=latents, seq_dim=2, process_group=self.get_context_parallel_group()
            )
        
        for _, t in enumerate(tqdm.tqdm(timesteps, desc="Denoising reference")):
            latent_model_input = latents
        ...
        if self.net.is_context_parallel_enabled:
            latents = cat_outputs_cp(
                latents, seq_dim=2, cp_group=self.get_context_parallel_group()
            )
