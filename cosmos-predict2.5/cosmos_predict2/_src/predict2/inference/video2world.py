...
def read_and_process_video_single_frame(
    video_path: str,
    resolution: list[int],
    num_video_frames: int,
    frame_index: int,
    ...
    # Extract the last frames_to_extract from input video
    target_frame_idx = available_frames - 5 + frame_index
    extracted_frames = video_tensor[:, target_frame_idx:target_frame_idx+1, :, :] 
    full_video[:, :frames_to_extract, :, :] = extracted_frames
    # log.info(f"Extracted last {frames_to_extract} frames from video (frames {start_idx} to {available_frames - 1})")
...
class Video2WorldInference:
    ...
    def generate_vid2world_compute_Mx(
        self,
        prompt: str,
        input_path: str | torch.Tensor | None,
        guidance: int = 7,
        ...
        if input_path is None or num_latent_conditional_frames == 0:
            ...
            if ext in _IMAGE_EXTENSIONS:
                ...
                log.info(f"Processing video input: {input_path}")
                vid_input = read_and_process_video_single_frame(
                    video_path=input_path,
    ...
    def generate_vid2world_attempt_denoise(
        self,
        prompt: str,
        input_path: str | torch.Tensor | None,
        guidance: int = 7,
        ...
        sample = self.model.denoise_from_reference(
            data_batch,
            noise_strength=noise_strength,  # NEW: how much noise to add
            n_sample=1,  # Generate one sample
            guidance=guidance,
            seed=seed,  # Fixed seed for reproducibility
            is_negative_prompt=True,  # Use classifier-free guidance
