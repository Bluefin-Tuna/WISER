...
class Driver_Switch:
    ...
    def __call__(self, policy, policy_2, steps=0, episodes=0):
        step, episode = 0, 0
        while step < steps or episode < episodes:
            step, episode = self._step(policy, policy_2, step, episode)

    def _step(self, policy, policy_2, step, episode):
        ...
        acts_1, self._state_1 = policy(obs, self._state_1, **self._kwargs)
        acts_2, self._state_2 = policy_2(obs, self._state_2, **self._kwargs)
        # print(step)
        if step == 0:
            self._swapped = False
        if self._swapped == False and self._state_1[3] == np.array(0): # Use the first policy
            acts = acts_1
           
            # self._state = self._state_1
        if self._swapped == True or self._state_1[3] != np.array(0):
            if self._swapped == False:
                print('Swapped...')
            self._swapped = True
            acts = acts_2
           
            # self._state = self._state_2
