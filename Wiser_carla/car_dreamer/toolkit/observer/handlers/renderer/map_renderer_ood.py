...
class MapRenderer:
    ...
    def render_synthetic_lane(self, waypoints: List[carla.Waypoint], offset_multiplier: float = 1.5):
        """Render a fake additional lane for visual augmentation."""
        # Offset by 1.5x lane width to place it adjacent
        lane_left = [lateral_shift(w.transform, -w.lane_width * offset_multiplier) for w in waypoints]
        lane_right = [lateral_shift(w.transform, -w.lane_width * (offset_multiplier + 1.0)) for w in waypoints]
        ...
        if len(polygon) > 2:
            ...
            # Add lane marking
            marking_points = [self.world_to_pixel(p) for p in lane_left]
            render_solid_line(self._surface, Color.ALUMINIUM_2, False, marking_points, 2)
    ...
    def render_topology(self, index=0):
        ...
        for i, waypoints in enumerate(set_waypoints):
            ...
            # self.render_synthetic_lane(waypoints, offset_multiplier=1.5)  # Add fake lane
            self.render_road_marking(waypoints)
            
            # if i > 375 or i < 350: # Avoid 300-400?
            #     self.render_road_marking(waypoints)
    ...
    def render_lane_marking(self, waypoints: List[carla.Waypoint], sign: int):
        ...
        marking_type = carla.LaneMarkingColor.White#carla.LaneMarkingType.NONE
        previous_marking_type = carla.LaneMarkingColor.White#carla.LaneMarkingType.NONE

        marking_color = carla.LaneMarkingColor.White#carla.LaneMarkingColor.Other
        previous_marking_color = carla.LaneMarkingColor.White#carla.LaneMarkingColor.Other
        ...
        for sample in waypoints:
            ...
            marking_color = carla.LaneMarkingColor.White#lane_marking.color
        ...
        for marking_type, marking_color, marking_points in markings_list:
            if marking_type == carla.LaneMarkingType.Solid:
                ...
                render_dotted_line(self._surface, carla.LaneMarkingColor.White, False, marking_points, 5)
    ...
    def render_lanes(self, waypoints: List[carla.Waypoint]):
        SHOULDER_COLOR = Color.BLACK#Color.ALUMINIUM_5
        PARKING_COLOR = Color.BLACK#Color.ALUMINIUM_4
        SIDEWALK_COLOR = Color.BLACK#Color.ALUMINIUM_3
    ...
    def render_traffic_signs(self):
        ...
        valid_stop_ids = [34, 40]
        stops = [stop for stop in stops if stop.id in valid_stop_ids]
